...
+---------------------------------------------------------------+"
		...
		if [ -f ./cidr/$name.cidr ]; then
			cat ./cidr/$name.cidr 2>/dev/null | cut -d" " -f3 | grep -E "^1.1.1.1$" >/dev/null && c="\033[1;37mFully loaded\033[0;40m" || c="\033[1;37mPartially loaded\033[0;40m"
		else
	...
	cat blacklists | grep -Ev "^#|^$" | tr -d "\r" |
		(
			while read -r line; do
				...
				if [ -f ./cidr/$name.cidr ]; then
					cat ./cidr/$name.cidr | cut -d" " -f3 | grep -E "^1.1.1.1$" >/dev/null &&
						{
							age=$([ -e ./cidr/$name.cidr ] && echo $(($(date +%s) - $(date -r ./cidr/$name.cidr +%s))) || echo 0)
							d=$(($age / 86400))
							if [[ $d -eq 7 ]]; then
...
|
+---------------------------------------------------------------+
You're alread -ry running the latest version of P2Partisan
\033[0;39m"
	else
		echo -e "\033[0;40m
+------------------------- P2Partisan --------------------------+
...
+-------------------------- Drop Logs --------------------------+"
	...
	if [ -z $1 ]; then
		...
		head -1 ./debug.log
		cat ./debug.log | grep "DEBUG-" | awk '{printf "%s %s %s ",$1,$2,$3;for (i=4;i<=NF;i++) if ($i~/(IN|OUT|SRC|DST|PROTO|SPT|DPT)=/) printf "%s ",$i;print ""}' | sed -e 's/PROTO=//g' -e 's/IN=/I=/g' -e 's/OUT=/O=/g' -e 's/SPT=/S=/g' -e 's/DPT=/D=/g' -e 's/SRC=/S=/g' -e 's/DST=/D=/g' | while read -r line; do
		...
		head -1 ./debug.log
		cat ./debug.log | grep "DEBUG-IN" | awk '{printf "%s %s %s ",$1,$2,$3;for (i=4;i<=NF;i++) if ($i~/(IN|OUT|SRC|DST|PROTO|SPT|DPT)=/) printf "%s ",$i;print ""}' | sed -e 's/PROTO=//g' -e 's/IN=/I=/g' -e 's/OUT=/O=/g' -e 's/SPT=/S=/g' -e 's/DPT=/D=/g' -e 's/SRC=/S=/g' -e 's/DST=/D=/g' | while read -r line; do
			[ $(($c % 2)) -eq 1 ] && printf "\e[100m"
			printf "%s\033[0m\n" "$line"
			c=$(($c + 1))
		done
		...
		head -1 ./debug.log
		cat ./debug.log | grep "DEBUG-OUT" | awk '{printf "%s %s %s ",$1,$2,$3;for (i=4;i<=NF;i++) if ($i~/(IN|OUT|SRC|DST|PROTO|SPT|DPT)=/) printf "%s ",$i;print ""}' | sed -e 's/PROTO=//g' -e 's/IN=/I=/g' -e 's/OUT=/O=/g' -e 's/SPT=/S=/g' -e 's/DPT=/D=/g' -e 's/SRC=/S=/g' -e 's/DST=/D=/g' | while read -r line; do
			[ $(($c % 2)) -eq 1 ] && printf "\e[100m"
			printf "%s\033[0m\n" "$line"
			c=$(($c + 1))
		done
			...
			(
				while read -r line; do
					...
					if [ "$(ipset swap $name $name 2>&1 | grep 'does not exist')" != "" ]; then
						[ -f ./cidr/$name.cidr ] && cat ./cidr/$name.cidr | cut -d" " -f3 | grep -E "^1.1.1.1$" >/dev/null && complete=1 || complete=0
...
|                |     __|  |_.---.-.----.|  |_
|                |__     |   _|  _  |   _||   _|
|        alread -ry |_______|____|___._|__|  |____| ed
|
+---------------------------------------------------------------+
| It appears like P2Partisan is alread -ry running. Skipping...
|
