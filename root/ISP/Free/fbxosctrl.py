...
class FbxConfiguration:
    ...
    def load(self, want_regapp):
        ...
        if self._reg_params is None:
            if not want_regapp:
                print('No registration params found in directory: {}'.
                      format(self._conf_path))
                print("You should launch 'fbxosctrl --regapp' once to \
                    register to the Freebox Server first.")
                sys.exit(0)
            else:
                # use wants to register:
                # this is normal not having reg params yet
                pass
        else:
    ...
    def _fetch_fbx_mdns_info_via_mdns(self):
        ...
        try:
            ...
            info['https_available'] = True \
                if serv_info.properties[b'https_available'] == b'1' else False
            info['https_port'] = int(serv_info.properties[b'https_port'])
            info['api_base_url'] = \
                serv_info.properties[b'api_base_url'].decode()
            info['api_version'] = \
                serv_info.properties[b'api_version'].decode()
            r.close()
        except Exception:
...
class FbxResponse:
    ...
    def __init__(self, jsonresp):
        ...
        elif self._resp.get('success') is not True and \
                self._resp['success'] is not False:
...
class FbxHttp():
    ...
    def get(self, uri, timeout=None, no_login=False):
        ...
        if requests.codes["ok"] != r.status_code:
            raise FbxException('GET error - http_status: {} {}'.format(
                r.status_code,
                r.text))
    ...
    def put(self, uri, data, timeout=None, no_login=False):
        ...
        # ensure status_code is 200, else raise exception
        if requests.codes['ok'] != r.status_code:
            raise FbxException('PUT error - http_status: {} {}'.format(
                r.status_code,
                r.text))
    ...
    def post(self, uri, data={}, timeout=None, no_login=False):
        ...
        # ensure status_code is 200, else raise exception
        if requests.codes['ok'] != r.status_code:
            raise FbxException('POST error - http_status: {} {}'.format(
                r.status_code,
                r.text))
    ...
    def _login(self):
        ...
        if not self._is_logged_in:
            ...
            log('challenge: {}, apptoken: {}'.format(self._challenge,
                                                     app_token))
            # Hashing token with key
            password = hmac.new(app_token.encode(),
                                self._challenge.encode(),
                                'sha1').hexdigest()
            uri = '/login/session/'
            payload = {'app_id': self._conf.app_desc.get('app_id'),
                       'password': password}
            # post it
            ...
            if resp.success:
                ...
                if not permissions.get('settings'):
                    print(
                        "Warning: permission 'settings' has not been allowed" +
                        ' yet in FreeboxOS server. This script may fail!')
            else:
        ...
        if 'granted' == status:
            ...
            print(
                'This track_id ({}) is unknown by Freebox Server: '.format(
                    track_id) +
                'you have to register again to Freebox Server to \
                    get a new app_id.')
        elif 'denied' == status:
            print(
                'This app has been denied by user on Freebox Server \
                    (track_id = {}).'
                .format(self._conf.reg_params.get('track_id')))
        elif 'timeout' == status:
            print(
                'Timeout occured for this app_id: you have to register again' +
                ' to Freebox Server to get a new app_id \
                    (current track_id = {}).'.format(track_id))
        else:
            print('Unexpected response: {}'.format(status))
        return status

    def register_app(self):
        """ Register this app to FreeboxOS to that user grants
        this apps via Freebox Server
        ...
        if register:
            ...
            if resp.success:
                ...
                print(input('Press Enter key once you have \
                    accepted on LCD screen: '))
                # check new status
                # (it seems to be mandatory to reach in 'granted' state)
                status = self.get_registration_status_diagnostic()
...
class FbxServiceSystem(FbxService):
    ...
    def get_system_info(self):
        ...
        print('Server info:')
        print(' - Model:     {}'.format(
            resp.result['model_info']['pretty_name']))
        ...
        for sensor in resp.result['sensors']:
            unit = 'Â°C' if sensor['id'].startswith('temp_') else ''
            print('   - {:20} {}{}'.format(sensor['name'] + ':',
                                           sensor['value'], unit))
        return True
...
class FbxServiceConnection(FbxService):
    ...
    def rate_to_human_readable(bps):
        ...
        if bps > 1000000:
            return '{:.2f} Mb/s ({:.1f} MB/s)'.format(
                bps/1000000, bps/1000000/8)
        elif bps > 1000:
    ...
    def get_line_ethernet_info(self):
        ...
        print(
            '   - Bandwidth:     {}'
            .format(FbxServiceConnection.rate_to_human_readable(
                resp.result['bandwidth_down'])))
        print(
            '   - Current rate:  {}'
            .format(FbxServiceConnection.rate_to_human_readable(
                resp.result['rate_down'])))
        print(' - Up:')
        print(
            '   - Bandwidth:     {}'
            .format(FbxServiceConnection.rate_to_human_readable(
                resp.result['bandwidth_up'])))
        print(
            '   - Current rate:  {}'
            .format(FbxServiceConnection.rate_to_human_readable(
                resp.result['rate_up'])))
    ...
    def _get_ftth_info(self):
        ...
        if resp.result['has_sfp'] is not True and \
                resp.result['sfp_present'] is not True:
...
class FbxServiceStorage(FbxService):
    ...
    def get_connected_drives(self):
        ...
        for drive in resp.result:
            ...
            print(' - {} ({}) | temp: {} | spining: {}'.format(model,
                                                               serial,
                                                               temp,
                                                               spinning))
        return True
        ...
        for drive in resp.result:
            ...
            for part in drive['partitions']:
                ...
                print(
                    ...
                    'total: {value:4.0f}{unit} |'.format(value=total,
                                                         unit=unit) +
                    ' used: {value:4.0f}{unit} |'.format(value=used,
                                                         unit=unit) +
                    ' free: {value:4.0f}{unit}'.format(value=avail,
                                                       unit=unit) +
                    ' ({value:.1f}{unit} free)'.format(value=free_percent,
                                                       unit='%'))
...
class FbxServiceWifi(FbxService):
    ...
    def _set_wifi_radio_state(self, set_on):
        ...
        try:
            ...
            if not set_on:
                # If we are connected using wifi, disabling wifi will
                # close connection thus PUT response will never be received:
    ...
    def get_wifi_planning(self):
        """ Get the current status of wifi: 1 means planning enabled,
        0 means no planning """
...
class FbxServiceCall(FbxService):
    ...
    def _get_calls_list(self, new_only):
        ...
        for call in calls:
            ...
            tag = '<' if status == 'outgoing'else '!' if status == 'missed' \
                else '>'
            naming = ' ({})'.format(name) if number != name else ''
            dur = ' - {}'.format(strdur) if status != "missed" and duration \
                else ''
                ...
                if len(dl[dl_type]):
                    for data in dl[dl_type]:
                        ...
                        if rx_rate > 1000000:
                            ...
                            rx_unit = 'o/s'
                        completion = data.get('rx_bytes') * 100 / \
                            data.get('size')
...
class FreeboxOSCli:
    ...
    def __init__(self, controller):
        ...
        self._parser = argparse.ArgumentParser(
            description='Command line utility to control some \
                FreeboxOS services.')
        # CLI related actions
        ...
        group.add_argument(
            ...
            action='store_true',
            help='register this app to FreeboxOS and save result in \
        ...
        group.add_argument(
            '--tlist',  # 't' stands for 'tÃ©lÃ©chargement'
            default=argparse.SUPPRESS,
            action='store_true',
    ...
    def parse_args(self, argv):
        ...
        if not os.path.isdir(conf_path):
            print('Configuration direcory does not exist: {}'.format(
                conf_path))
            sys.exit(1)
