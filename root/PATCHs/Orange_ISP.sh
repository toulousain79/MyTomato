...
declare gsScriptName gsDirOverLoad gsOrange_FTI i
...
sed -i '/exec busybox udhcpc/d' /tmp/script_init
{
	echo "# Orange DHCP Mode"
	echo "echo 'exec busybox udhcpc -O 0x4d -O 0x5a -x 0x4d:2b46535644534c5f6c697665626f782e496e7465726e65742e736f66746174686f6d652e4c697665626f7833 -x 0x5a:00000000000000000000001a0900000558010341010d6674692f${HEXA} \"$*\"' > /sbin/udhcpc"
} >/tmp/script_init
...
echo "bash /opt/etc/orange_ack_script_fire.sh" >/tmp/script_fire
