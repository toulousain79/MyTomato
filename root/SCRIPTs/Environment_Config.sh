...
if (! /opt/bin/mount -l | grep -q '/tmp/home/root'); then
    if [ ! -f /tmp/home/root/.uuid ]; then
        logger -p user.notice -t -s "| ${gsScriptName} | Clean /tmp/home/root/"
        rm -rRf /tmp/home/root/* && rm -rRf /tmp/home/root/.??*
    fi
    if [ ! -f /opt/root/.uuid ]; then
        logger -p user.notice -t -s "| ${gsScriptName} | Clean /opt/root/"
        rm -rRf /opt/root/* && rm -rRf /opt/root/.??*
    fi
    logger -p user.notice -t -s "| ${gsScriptName} | Mount /opt/MyTomato/root to /tmp/home/root"
...
#### Locales
if [ -n "${gsLocales}" ]; then
    logger -p user.notice -t -s "| ${gsScriptName} |  Add locales '${gsLocales}'"
    /opt/bin/localedef.new -c -f UTF-8 -i "${gsLocales}" "${gsLocales}.UTF-8"
fi
if [ -n "${gsTimezone}" ]; then
    logger -p user.notice -t -s "| ${gsScriptName} |  Add timezone '${gsTimezone}'"
