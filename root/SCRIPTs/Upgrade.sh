...
if [ -f "${gsOpkgPackagesList}" ]; then
    while read -r line; do
        ...
        (! opkg list-installed | grep -q "${sPackage}") && opkg install "${sPackage}" \
            logger -p user.notice -t "| ${gsScriptName} | EntWare install package '${sPackage}'"
    done <"$gsOpkgPackagesList"
fi

logger -p user.notice -t "| ${gsScriptName} | EntWare generate pakages installed list"
...
if [ "${gbRepoUpgrade_Enable:?}" -eq 1 ]; then
    ...
    logger -p user.notice -t "| ${gsScriptName} | Update /opt/MyTomato via GitHub"
    git fetch origin
...
if [ ! -d /opt/usr/local/dnscrypt-proxy ]; then
    logger -p user.notice -t "| ${gsScriptName} | Git clone https://github.com/jedisct1/dnscrypt-proxy.git"
    git clone git://github.com/jedisct1/dnscrypt-proxy.git "${gsDirDnscrypt:?}"
else
    cd "${gsDirDnscrypt:?}" || exit 1
    logger -p user.notice -t "| ${gsScriptName} | Update ${gsDirDnscrypt} via GitHub"
    git fetch origin
    git reset --hard origin/master
    # git pull origin master
...
if [ -f "${gsDirOverLoad}"/dnscrypt/generate-domains-blacklists/generate-domains-blacklist.py ] && [ -f "${gsDirOverLoad}"/dnscrypt/generate-domains-blacklists/domains-blacklist.conf ]; then
    ...
    logger -p user.notice -t "| ${gsScriptName} | Generate 'blacklists.txt' with 'generate-domains-blacklist.py'"
    python generate-domains-blacklist.py -c domains-blacklist.conf >list.txt.tmp && mv -f list.txt.tmp blacklists.txt
fi

#### SCRIPTs
logger -p user.notice -t "| ${gsScriptName} | Chmod +x to ${gsDirScripts}/*"
chmod +x "${gsDirScripts}"/*
logger -p user.notice -t "| ${gsScriptName} | Chmod +x to /opt/MyTomato/P2Partisan/p2partisan.sh"
